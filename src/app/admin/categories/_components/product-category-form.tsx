...
import { addProductCategory, editProductCategory } from '@/app/admin/categories/actions'
...
const ProductCategoryForm = ({ slug, productCategory = {} }: ProductCategoryFormProps) => {
  ...
  const handleProductCategoryFormSubmit = (values: AddProductCategoryPayload) => {
    ...
    Object.keys(productCategory).length > 0 ? handleEditProductCategory(payload) : handleAddProductCategory(payload)
  }

  useEffect(() => {
    if (!Object.keys(productCategory).length) return
    reset({
