        ...
        <ul className='relative py-3'>
          {CATEGORIES.map((category, index) => (
            <Fragment key={new Date().getTime() + index}>
              ...
              {/* Sub category */}
              {hoveredCate?.title === category.title && (category.items as [])?.length > 0 && (
                <div className='absolute left-[calc(100%+12px)] top-0 p-5 flex gap-5 bg-white w-[868px] rounded-[3px] shadow-md'>
